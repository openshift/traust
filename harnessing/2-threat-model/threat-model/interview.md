...
listen, ground their answers in the code where you can, and emit the model
file (`<model-file>` = `<repo>-threat-model.md`; naming rule in `SKILL.md`)
per `schema.md`.
...
- `--seed <model-file>` (optional): a prior `bootstrap` output. If
...
Every fact you write into the model file carries one of two tags in your
working notes:
...
- "What data does it hold or pass through that would be bad to lose, leak, or
  tamper with?" → assets table. For each sensitive asset, follow up once:
  "Is any of that regulated — GDPR, HIPAA, PCI, export control? What do the
  actual records look like?" → the optional `regulatory_scope` /
  `example_records` columns (schema.md section 2). One follow-up, not an
  audit; leave cells empty if the owner doesn't know.
...
Then put a face on the attacker — ask once, early in Q2: **"Who actually
wants this? Of these, which fit: nation-state, organized crime, a malicious
or careless insider, a competitor, someone attacking your supply chain, or
just opportunists scanning the internet?"** Capture 2-4 personas with the
owner's one-sentence reasoning; they become the `### Threat actor
landscape` subsection of section 1 (schema.md). Personas are narrative
color — the section 4 `actor` column stays the access-position enum.
...
Write `<target-dir>/<model-file>` per `schema.md` — including the
section 1 executive summary (jargon-free, written last), the threat actor
landscape subsection from Q2, and section 9 scenarios as validated with the
owner in Q4. Set `## 7. Provenance`:
...
Run the deterministic gate and fix every ERROR until it passes:
