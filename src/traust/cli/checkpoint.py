...
# State I/O is confined under the current working directory — run this tool
# from the directory you want to scope to (plan P3). No env override.
_ROOT = Path().resolve()


# Dot-components a checkpoint path may legitimately traverse: the
# skills' own scratch dirs. Everything else dot-prefixed (.git — where
# an appended fsmonitor/hook line is code execution — .claude, .ssh via
# a symlinked subdir, ...) is refused (self-audit -012).
_DOT_ALLOW = re.compile(r"^\.[a-z][a-z-]*-state$")
...
def _confine(p: str | Path, *, must_end: str | None = None) -> Path:
    ...
    keeps the blast radius at the repo being scanned — and inside that
    root, dot-paths (.git/config!) and symlinked targets are refused."""
    r = Path(p).resolve()
    if not r.is_relative_to(_ROOT):
        print(f"checkpoint: refusing path outside {_ROOT}: {p}", file=sys.stderr)
        raise SystemExit(2)
    for part in r.relative_to(_ROOT).parts:
        if part.startswith(".") and not _DOT_ALLOW.match(part):
            print(
                f"checkpoint: refusing dot-path component {part!r} in {p} "
                f"(only *-state scratch dirs may be dot-dirs)",
                file=sys.stderr,
            )
            raise SystemExit(2)
    if Path(p).is_symlink() or r.is_symlink():
        print(f"checkpoint: refusing symlinked target: {p}", file=sys.stderr)
        raise SystemExit(2)
...
def _write_progress(state_dir: Path, *, status: str, key: str, n: int, shards: list[str]) -> None:
    _atomic_write(
...
def cmd_shard(argv: list[str]) -> int:
    ...
    prog["updated"] = datetime.now(UTC).isoformat()
...
if __name__ == "__main__":
    ...
    raise SystemExit(main(["admin", "checkpoint", *sys.argv[1:]]))
